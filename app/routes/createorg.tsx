...
export async function action({ request, context }: Route.ActionArgs) {
    ...
    // add org to creator
    await addOrgToUser(userInfo._id, orgInfo._id.toString());

    // send invites to member
    const invitePromises = [];
    for (const memberEmail of members) {
        invitePromises.push(
            addInvitations({
                email: memberEmail?.toString(),
                orgId: orgInfo?._id.toString(),
    ...
    try {
        ...
        logger.error(error, 'Unable to invite members');
        return { error: 'Unable to invite members' };
    }
    return redirect(`/onboarding/${orgInfo?._id.toString()}`);
}
    ...
    return (
        <div className={`bg-primary ${mapTextColor('primary')} flex min-h-screen flex-col font-sans`}>
            ...
            <div className="container-padding-x flex flex-1 flex-col items-center justify-center gap-y-8">
                ...
                {data?.error && <div className="container-padding-x flex flex-1 flex-col items-center justify-center gap-y-8 text-red-700">Error {data?.error}</div>}
            </div>
        </div>
    );
}
